...
The `home-routes` handler in the `semantic-web-ws.routes.home` namespace
...
The `home-page` function will in turn call the `semantic-web-ws.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `semantic-web-ws.handler` namespace:
...
Request middleware functions are located under the `semantic-web-ws.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `semantic-web-ws.dev-middleware` namespace found in
